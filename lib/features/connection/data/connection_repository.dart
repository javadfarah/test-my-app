...
import 'package:spotify/features/connection/model/connection_failure.dart';
import 'package:spotify/features/connection/model/connection_status.dart';
import 'package:spotify/features/profile/data/profile_path_resolver.dart';
import 'package:spotify/features/profile/model/profile_entity.dart';
import 'package:spotify/features/settings/data/config_option_repository.dart';
import 'package:spotify/features/settings/notifier/warp_option/warp_option_notifier.dart';
import 'package:spotify/hiddifycore/hiddify_core_service.dart';
import 'package:spotify/singbox/model/singbox_config_option.dart';
import 'package:spotify/singbox/model/core_status.dart';
  ...
  TaskEither<ConnectionFailure, Unit> connect(ProfileEntity activeProfile, bool disableMemoryLimit) => setup().flatMap(
    (_) => applyConfigOption(activeProfile).flatMap(
      (_) => TaskEither.tryCatch(() async {
        final configFile = profilePathResolver.file(activeProfile.id);
        await ProfileConfigSanitizer.sanitizeFile(configFile);
        return unit;
      }, (err, st) => ConnectionFailure.unexpected(err, st)).flatMap(
